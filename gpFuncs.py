...
class featureLayer(object):
    ...
    def iterateFeatures(self):
        ...
        if desc.hasOID == 1:
            ...
            #use new da cursor and with block as they are much faster and
            #the with block make sure everything is cleaned up
            #also using the oid@ token to just return the oid data
            with arcpy.da.SearchCursor(self.path,['OID@']) as scur:
                for srow in scur:
                    objId = srow[0]
                    sql = '"' + oidField + '"' + " = " + objId
...
##                icur_field_param_list.append(icur_field)
            ...
            with arcpy.da.InsertCursor(featVerticesPnts, ['*']) as icur:
                ...
                with arcpy.da.SearchCursor(self.path, ['*'], '#', '#', True) as cursor:
                    for row in cursor:
...
##                            icur.insertRow(insert_param_list)
    ...
    def featureVerticesToPoints(self, featureid, workspace=None, first_and_last=None, mark_ordinal=None):
        ...
        try:
            arcpy.CreateFeatureclass_management(os.path.dirname(featVerticesPnts),
                os.path.basename(featVerticesPnts), "POINT", '#', "#", "#", self.path)
...
##            outPointRows = arcpy.InsertCursor(featVerticesPnts)
            #da cursor in with statement
            with arcpy.da.InsertCursor(featVerticesPnts, ['SHAPE@xy','featureid']) as outPointRows:
##                flds = outPointRows.fields
                with arcpy.da.SearchCursor(self.path, ['SHAPE@xy','featureid'], '#', '#', True) as inPolylineRows:
                    for inPolylineRow in inPolylineRows:
                        #just insert the row minus the oid field, which is the first item.
                        outPointRows.insertRow(inPolylineRow)
